...
class Notes extends Component {
  renderNote(note) {
    ...
    if (moment(note.published).isSame(moment(), "day"))
      time = moment(note.published).fromNow();
    return (
  ...
  render() {
    ...
    return (
      <div className="leftpane">
        ...
        {notesCount == 0 ? (
          <div className="notes-index-sidebar">No Notes</div>
        ) : (
          <div />
        )}
        <div className="columns">{notes.map(this.renderNote.bind(this))}</div>
      </div>
    );
  }
}
